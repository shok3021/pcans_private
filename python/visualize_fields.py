...
GLOBAL_PLOT_RANGES = {
    ...
    # --- [修正] eV単位に合わせてレンジを拡大 (必要に応じて調整してください) ---
    'Te': (0, 1e7),   # 例: 0 eV 〜 2000 eV
    'Ti': (0, 2e5),   # 例: 0 eV 〜 2000 eV
    # -------------------------------------------------------------
...
def process_timestep(timestep):
    ...
    # ★★★ 温度データの読み込み ★★★
    # ここで読み込まれるデータはすでに eV 単位
    Te_raw_eV = load_2d_moment_data(timestep, 'electron', 'T')
    Ti_raw_eV = load_2d_moment_data(timestep, 'ion', 'T')
    ...
    # --- [修正] 温度の取り扱い ---
    # 元のデータがすでに eV なので、VA0で割ってはいけません。そのまま代入します。
    Te = Te_raw_eV
    Ti = Ti_raw_eV
    # --------------------------
    ...
    plot_components = [
        ...
        ('Te', Te, 'Electron Temperature', r'$T_e$ (eV)', plt.cm.plasma), # ★ eV単位
        ('ni', ni, 'Ion Density', r'$n_i$ (Counts)', plt.cm.viridis),
        ('Ti', Ti, 'Ion Temperature', r'$T_i$ (eV)', plt.cm.plasma),      # ★ eV単位
    ...
    combined_plots = [
        ...
        ('Ti', Ti, '(p) $T_i$', r'$T_i$ (eV)', plt.cm.plasma),          # ★ eV単位
        ...
        ('Te', Te, '(t) $T_e$', r'$T_e$ (eV)', plt.cm.plasma),          # ★ eV単位
    ]
