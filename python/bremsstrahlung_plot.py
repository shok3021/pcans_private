...
# =======================================================
BREMS_DATA_DIR = os.path.join(os.path.abspath('.'), 'bremsstrahlung_maxwell_intensity')
...
OUTPUT_DIR = os.path.join(os.path.abspath('.'), 'bremsstrahlung_maxwell_plots') # 出力先フォルダ名を変更
...
def plot_brems_panel_subplot(ax, X, Y, Intensity, Bx, By, subtitle, omega_t_str, show_ylabel=True):
    ...
    cf = ax.contourf(X, Y, Intensity, levels=levels, cmap=cmap, extend='max')
